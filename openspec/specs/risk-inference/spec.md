...
The system re-scores the in-scope fleet from persisted telemetry using the
trained model, replacing risk score, risk level, explanation, the three top
features and the six-day trend.

Scoring happens in a dedicated stateless service that owns the model artefact
and has no database access, so the model's ~300 MB of dependencies stay out of
the API image and the scoring hop is visible as its own span in a trace.

Two properties shape everything here, because both fail silently rather than
loudly. The model was trained on absolute temperatures, so the Celsius stored by
`telemetry-ingestion` is converted at exactly one boundary and range-checked
before anything is scored — a wrong unit produces plausible, wrong scores and no
error. And an elevator with no recent telemetry is skipped rather than scored
with defaults, so a unit that stopped reporting reads as stale rather than as
healthy.

Scope boundary: this capability covers scoring and persisting the result. It
does not retrain the model, and it does not schedule itself — a run is
triggered.
