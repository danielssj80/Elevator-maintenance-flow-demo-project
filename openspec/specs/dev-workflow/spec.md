...
A documented, production-safe, two-track development workflow. **Track A (Claude Code on the web)** is for non-Docker work — Python/ML (e.g. M1 offline training), code edits, docs, and opening PRs. **Track B (local machine or the dedicated dev EC2)** is for the full Docker Compose stack and the backend/integration/E2E tests. The split exists because the web sandbox cannot practically run docker-in-docker. Production is protected throughout: `main` branch protection plus the web sandbox's branch-scoped push mean a cloud session can only reach production through a reviewed PR merge.

## Requirements

### Requirement: Portable dev-stack setup primitive
The project SHALL provide an idempotent setup script (`scripts/dev-setup.sh`) that prepares the development/test Docker stack on any Linux host with a working Docker daemon and a Compose CLI. It SHALL auto-detect the Compose CLI (prefer `docker compose` v2, fall back to `docker-compose` v1), pull the database image, build the application images, be safe to run repeatedly, and NOT start services or mutate persisted data. It is intended for **local machines and the dedicated dev EC2**; it is not used in the Claude Code web sandbox, which cannot run Docker builds (see "Documented two-track development workflow").
