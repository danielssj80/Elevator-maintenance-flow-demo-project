...
The system emits traces, metrics and logs over OTLP so that a request can be
followed across services, a scheduled job can be shown to have run, and a
dependency failure that the application deliberately absorbs is still visible.

Scope boundary: OpenTelemetry carries **operational** signals only — how the
system behaved. Domain data (sensor readings, risk scores) lives in PostgreSQL
and is served by the API; it is never routed through the telemetry pipeline. A
small number of derived, low-cardinality gauges bridge the two so fleet state
can appear on an operational dashboard.

The observability stack (Collector, Grafana, Prometheus, Tempo, Loki) runs
locally. Production is unchanged and exposes none of it.
