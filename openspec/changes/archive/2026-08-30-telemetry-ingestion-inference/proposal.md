...
- **Security**: `POST /api/telemetry/readings` and `POST /api/inference/run` are unauthenticated write endpoints, and `docker-compose.prod.yml` auto-deploys on merge to `main`. Production router-gating is a requirement of this change, not a follow-up. The gate is **fail-closed**: `deployment_environment` defaults to `production`, because `docker-compose.prod.yml` sets it nowhere and loads an out-of-repo env file, so a default of `local` left the gate open in the one environment it protects. `docker-compose.prod.yml` now also states it explicitly, and every non-production environment declares itself.
