...
- Add `POST /api/telemetry/readings` (batch ingest, max 1000), `GET /api/telemetry/readings` (windowed query) and `POST /api/inference/run` (trigger a re-scoring run).
...
- **Database**: one new table with two indexes. Existing tables unchanged; `elevators`, `elevator_features` and `elevator_trend_points` gain new *rows* during a run, not new columns.
